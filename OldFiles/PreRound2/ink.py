...
class Trader:
    ...
    def kelp_strategy(self, state: TradingState, order_depth: OrderDepth, trader_data: Dict) -> List[Order]:
        ...
        if spread >= 2:
          buy_price = int(mid_price - 3)
          sell_price = int(mid_price + 3)
          if position < LIMIT:
              orders.append(Order(product, buy_price, LIMIT - position))
          if position > -LIMIT:
              orders.append(Order(product, sell_price, position + LIMIT))
    ...
    def ink_strategy(self, state: TradingState, order_depth: OrderDepth, trader_data: Dict):
        ...
        if (position == -50 or intensity >= risk_threshold) and best_ask:
            buy_qty = min(-position, -best_ask_volume)
            if buy_qty > 0:
                orders.append(Order(product, best_ask, buy_qty))
                print(f"{product} COVERING SHORT {buy_qty} @ {best_ask}")
                position += buy_qty

        if best_bid is not None and position > -LIMIT:
            sell_qty = LIMIT + position
            orders.append(Order(product, best_bid, -sell_qty))
            print(f"{product} SHORTING {sell_qty} @ {best_bid}")
            position -= sell_qty
