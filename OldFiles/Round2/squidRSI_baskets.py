...
class Trader:
    ...
    def ink_strategy(self, state: TradingState, order_depth: OrderDepth, trader_data: Dict) -> List[Order]:
        ...
        if best_bid == 0 or best_ask == 0:
            return orders  # Skip if no liquidity

        mid_price = (best_bid + best_ask) / 2
        mid_prices.append(mid_price)
        mid_prices = mid_prices[-100:]  # Keep only last 100 prices
        ...
        if len(mid_prices) < RSI_WINDOW + 1:
            self.ink_data = {"mid_prices": mid_prices}
            return orders

        # --- Calculate RSI ---
        deltas = np.diff(mid_prices[-(RSI_WINDOW + 1):])
        ...
        print(f"INK: RSI={rsi:.2f}, Price={mid_price:.2f}")

        # --- ENTRY LOGIC ---
        if position < LIMIT and rsi < ENTRY_RSI:
            qty = LIMIT - position
            orders.append(Order(product, int(mid_price), qty))
            print(f"INK: BUY {qty} @ {int(mid_price)} (RSI={rsi:.2f})")

        elif position > -LIMIT and rsi > (100 - ENTRY_RSI):  # Short entry
            qty = position + LIMIT
            orders.append(Order(product, int(mid_price), -qty))
            print(f"INK: SELL {qty} @ {int(mid_price)} (RSI={rsi:.2f})")

        # --- EXIT LOGIC ---
        elif position > 0 and rsi > EXIT_RSI:
            orders.append(Order(product, int(mid_price), -position))
            print(f"INK: EXIT LONG {position} @ {int(mid_price)} (RSI={rsi:.2f})")

        elif position < 0 and rsi < (100 - EXIT_RSI):
            orders.append(Order(product, int(mid_price), -position))
            print(f"INK: EXIT SHORT {position} @ {int(mid_price)} (RSI={rsi:.2f})")

        # --- Save updated mid prices to trader state ---
        self.ink_data = {"mid_prices": mid_prices}
        return orders
