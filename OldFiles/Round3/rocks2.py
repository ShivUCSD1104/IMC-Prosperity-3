...
class Trader:
    ...
    def bsm(self, S, K, T, sigma):
        """
        Black-Scholes European call option price using only numpy.
        Includes calculation of delta.
        """
        if T <= 0 or sigma == 0:
    ...
    def volcanic_voucher_strategy(self, state: TradingState, order_depth: OrderDepth, product: str, trader_data: Dict) -> List[Order]:
        ...
        T = 5 / 365 - (state.timestamp / 1000000) / 365
        T = max(T, 0.0001)  # Avoid division by zero

        # Black-Scholes call price and delta
        model_price, delta = self.bsm(fair_price, STRIKE, T, volatility)
